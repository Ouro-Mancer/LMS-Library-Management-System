...
const Catalog = () => {
  ...
  const [borrowedBookName, setBorrowedBookName] = useState("");
  const [readBook, setReadBook] = useState({});

  const openReturnBookPopup = (bookId, email, bookName) => {
    const book = books.find((book) => book._id === bookId);
    setReadBook(book);
    console.log(book);
    setBorrowedBookId(bookId);
    setBorrowedBookName(bookName);
    setEmail(email);
    dispatch(toggleReturnBookPopup());
  };

  const calculateFine = (createdAt) => {
    ...
    if (today > dueDate) {
      ...
      return diffDays * 5; // ₹10 per day fine
    }
    return 0;
  };
              ...
              <tbody>
                {booksToDisplay.map((book, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2">{book?.book?.bookName}</td>


                    <td className="px-4 py-2">₹{calculateFine(book.createdAt)}</td>
                    ...
                    <td className="px-4 py-2">
                      {book.returnDate ? (
                        ...
                        <PiKeyReturnBold
                          className="w-6 h-6"
                          onClick={() =>
                            openReturnBookPopup(book?.book?._id, book?.user?.email, book?.book?.bookName)
                          }
                        />
                      )}
                    </td>
